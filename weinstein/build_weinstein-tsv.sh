...
rm weinstein.tsv
rm weinstein_noOTT.tsv
rm weinstein_nohit.tsv
echo -e "sourceTaxonId\tsourceTaxonName\tinteractionTypeName\treferenceCitation" >> weinstein.tsv 
echo -e "sourceTaxonId\tsourceTaxonName\tinteractionTypeName\tParentOTT\treferenceCitation" >> weinstein_noOTT.tsv

while IFS='' read -r line || [[ -n "$line" ]]; do
    hit=$(grep "$line" "$2" | head -1 | awk -F "|" '{OFS="\t"; print $1,$3,"parasiteOf","weinstein2016"}' | tr -s '\t')
    if [[ $hit ]]; then
    echo "$hit" >> weinstein.tsv
    ...
    if [ ${#hit2} -ge 3 ]; then
        ...
        if [[ $hit3 ]]; then
            echo "$hit4" >> weinstein_noOTT.tsv
        else
            echo "$line" >> weinstein_nohit.tsv
        fi
    ((ottid++))
    else
    echo "$line" >> weinstein_nohit.tsv
