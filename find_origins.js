...
db.query(`  FOR doc IN edges_otl_sub
...
function calculateOrigins(cursor) {
    ...
    cursor.all().then(docs => {
        ...
        if (doc1.pi < 0.5 && doc2.pi > 0.5){
            db.query(`UPDATE "${doc1._key}" WITH {  origin_from: 1} in nodes_otl_sub`);
            db.query(`UPDATE "${doc2._key}" WITH {  origin_to: 1,
                                                    originID: ${counter}} in nodes_otl_sub`);
            counter++;}
        if (doc1.pi > 0.5 && doc2.pi < 0.5){
            
            db.query(`UPDATE "${doc1._key}" WITH {  loss_from: 1} in nodes_otl_sub`);
            db.query(`UPDATE "${doc2._key}" WITH {  loss_to: 1,
                                                    lossID: ${counter2}} in nodes_otl_sub`);
            counter2++;}
        }
