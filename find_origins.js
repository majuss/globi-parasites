...
function calculateOrigins(cursor) {
    ...
    cursor.all().then(docs => {
        ...
        if (doc1.pi < 0.5 && doc2.pi > 0.5){
            db.query(`UPDATE "${doc1._key}" WITH {  origin_from: 1} in nodes_otl`);
            db.query(`UPDATE "${doc2._key}" WITH {  origin_to: 1,
                                                    originID: ${counter}} in nodes_otl`);
            counter++;}
        if (doc1.pi > 0.5 && doc2.pi < 0.5){
            
            db.query(`UPDATE "${doc1._key}" WITH {  loss_from: 1} in nodes_otl`);
            db.query(`UPDATE "${doc2._key}" WITH {  loss_to: 1,
                                                    lossID: ${counter2}} in nodes_otl`);
            counter2++;}
        }
