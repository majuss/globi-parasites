...
async function counting() {
    ...
    FOR v,e IN 1..100 outbound 'nodes_otl_weinonly/304358' edges_otl_weinonly
    ...
    Object.keys(kingdomcount._result).forEach(async function (key) {
        ...
        return count(
        FOR v,e IN 1..100 outbound 'nodes_otl_weinonly/${kingdomcount._result[key]._key}' edges_otl_weinonly
        ...
        Object.keys(phyllacount._result).forEach(async function (key) {
            let countp = await db.query(`
            return count(
            FOR v,e IN 1..100 outbound 'nodes_otl_weinonly/${phyllacount._result[key]._key}' edges_otl_weinonly
            FILTER v.origin_to == 1
            RETURN v)`);
            let countf = await db.query(`
            return count(
            ...
            Object.keys(classcount._result).forEach(async function (key) {
                let countp = await db.query(`
                return count(
                FOR v,e IN 1..100 outbound 'nodes_otl_weinonly/${classcount._result[key]._key}' edges_otl_weinonly
                FILTER v.origin_to == 1
                RETURN v)`);
                let countf = await db.query(`
                return count(
                ...
                Object.keys(ordercount._result).forEach(async function (key) {
                    let countp = await db.query(`
                    return count(
                    FOR v,e IN 1..100 outbound 'nodes_otl_weinonly/${ordercount._result[key]._key}' edges_otl_weinonly
                    FILTER v.origin_to == 1
                    RETURN v)`);
                    let countf = await db.query(`
                    return count(
                    ...
                    Object.keys(familycount._result).forEach(async function (key) {
                        let countp = await db.query(`
                        return count(
                        FOR v,e IN 1..100 outbound 'nodes_otl_weinonly/${familycount._result[key]._key}' edges_otl_weinonly
                        FILTER v.origin_to == 1
                        RETURN v)`);
                        let countf = await db.query(`
                        return count(
                        ...
                        Object.keys(genuscount._result).forEach(async function (key) {
                            let countp = await db.query(`
                            return count(
                            FOR v,e IN 1..100 outbound 'nodes_otl_weinonly/${genuscount._result[key]._key}' edges_otl_weinonly
                            FILTER v.origin_to == 1
                            RETURN v)`);
                            let countf = await db.query(`
                            return count(
