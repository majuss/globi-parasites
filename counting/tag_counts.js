...
async function counting() {
    let kingdomcount = await db.query(`FOR v,e IN 1..100 outbound 'otl_parasites_nodes/304358' otl_parasites_edges
          filter v.rank == 'kingdom'
          RETURN v`);

    Object.keys(kingdomcount._result).forEach(async function (key) {

        let countp = await db.query(`return count(
    FOR v,e IN 1..100 outbound 'otl_parasites_nodes/${kingdomcount._result[key]._key}' otl_parasites_edges
    FILTER v.parasite == 1
    RETURN v)`);
        let countf = await db.query(`return count(
    FOR v,e IN 1..100 outbound 'otl_parasites_nodes/${kingdomcount._result[key]._key}' otl_parasites_edges
    FILTER v.freeliving == 1
    RETURN v)`);
        db.query(`UPDATE "${kingdomcount._result[key]._key}" WITH {
    nr_parasites: ${countp._result},
    nr_freeliving: ${countf._result}
    } IN otl_parasites_nodes_bak`)

        console.log(kingdomcount._result[key].name, kingdomcount._result[key]._key, countp._result, countf._result); // kingdoms 


        let phyllacount = await db.query(`FOR v,e IN 1..100 outbound 'otl_parasites_nodes/${kingdomcount._result[key]._key}' otl_parasites_edges
        ...
        Object.keys(phyllacount._result).forEach(async function (key) {
            ...
            FILTER v.freeliving == 1
            RETURN v)`);
            db.query(`UPDATE "${phyllacount._result[key]._key}" WITH {
            nr_parasites: ${countp._result},
            nr_freeliving: ${countf._result}
            } IN otl_parasites_nodes_bak`)
            ...
            let classcount = await db.query(`FOR v,e IN 1..100 outbound 'otl_parasites_nodes/${phyllacount._result[key]._key}' otl_parasites_edges
            filter v.rank == 'class'
            RETURN v`);
            ...
            Object.keys(classcount._result).forEach(async function (key) {
                ...
                FILTER v.freeliving == 1
                RETURN v)`);
                db.query(`UPDATE "${classcount._result[key]._key}" WITH {
                nr_parasites: ${countp._result},
                nr_freeliving: ${countf._result}
                } IN otl_parasites_nodes_bak`)
                ...
                console.log(classcount._result[key].name, classcount._result[key]._key, countp._result, countf._result); // class
                    ...
                    let ordercount = await db.query(`FOR v,e IN 1..100 outbound 'otl_parasites_nodes/${classcount._result[key]._key}' otl_parasites_edges
                    filter v.rank == 'order'
                    RETURN v`);
                ...
                FILTER v.freeliving == 1
                RETURN v)`);
                db.query(`UPDATE "${ordercount._result[key]._key}" WITH {
                nr_parasites: ${countp._result},
                nr_freeliving: ${countf._result}
                } IN otl_parasites_nodes_bak`)
                ...
                console.log(ordercount._result[key].name, ordercount._result[key]._key, countp._result, countf._result); // order
                     ...
                     let familycount = await db.query(`FOR v,e IN 1..100 outbound 'otl_parasites_nodes/${ordercount._result[key]._key}' otl_parasites_edges
                ...
                FILTER v.freeliving == 1
                RETURN v)`);
                db.query(`UPDATE "${familycount._result[key]._key}" WITH {
                nr_parasites: ${countp._result},
                nr_freeliving: ${countf._result}
                } IN otl_parasites_nodes_bak`)
                ...
                console.log(familycount._result[key].name, familycount._result[key]._key, countp._result, countf._result); // family
                      ...
                      let genuscount = await db.query(`FOR v,e IN 1..100 outbound 'otl_parasites_nodes/${familycount._result[key]._key}' otl_parasites_edges
                    filter v.rank == 'family'
                    RETURN v`);
                ...
                FILTER v.freeliving == 1
                RETURN v)`);
                db.query(`UPDATE "${genuscount._result[key]._key}" WITH {
                nr_parasites: ${countp._result},
                nr_freeliving: ${countf._result}
                } IN otl_parasites_nodes_bak`)
