...
async function counting() {
    ...
    Object.keys(kingdomcount._result).forEach(async function (key) {
        ...
        return count(
        FOR v,e IN 1..100 outbound 'otl_parasites_nodes_nowein/${kingdomcount._result[key]._key}' otl_parasites_edges_nowein
        FILTER v.origin2 == 1
        RETURN v)`);
        ...
        Object.keys(phyllacount._result).forEach(async function (key) {
            let countp = await db.query(`
            return count(
            FOR v,e IN 1..100 outbound 'otl_parasites_nodes_nowein/${phyllacount._result[key]._key}' otl_parasites_edges_nowein
            FILTER v.origin2 == 1
            RETURN v)`);
            let countf = await db.query(`
            return count(
            ...
            Object.keys(classcount._result).forEach(async function (key) {
                let countp = await db.query(`
                return count(
                FOR v,e IN 1..100 outbound 'otl_parasites_nodes_nowein/${classcount._result[key]._key}' otl_parasites_edges_nowein
                FILTER v.origin2 == 1
                RETURN v)`);
                let countf = await db.query(`
                return count(
                ...
                Object.keys(ordercount._result).forEach(async function (key) {
                    let countp = await db.query(`
                    return count(
                    FOR v,e IN 1..100 outbound 'otl_parasites_nodes_nowein/${ordercount._result[key]._key}' otl_parasites_edges_nowein
                    FILTER v.origin2 == 1
                    RETURN v)`);
                    let countf = await db.query(`
                    return count(
                    ...
                    Object.keys(familycount._result).forEach(async function (key) {
                        let countp = await db.query(`
                        return count(
                        FOR v,e IN 1..100 outbound 'otl_parasites_nodes_nowein/${familycount._result[key]._key}' otl_parasites_edges_nowein
                        FILTER v.origin2 == 1
                        RETURN v)`);
                        let countf = await db.query(`
                        return count(
                        ...
                        Object.keys(genuscount._result).forEach(async function (key) {
                            let countp = await db.query(`
                            return count(
                            FOR v,e IN 1..100 outbound 'otl_parasites_nodes_nowein/${genuscount._result[key]._key}' otl_parasites_edges_nowein
                            FILTER v.origin2 == 1
                            RETURN v)`);
                            let countf = await db.query(`
                            return count(
