...
db.query(`  FOR doc IN otl_parasites_edges_nowein 
...
function calculateOrigins(cursor) {
    ...
    cursor.all().then(docs => {
        ...
        if (doc1.pi < 0.5 && doc2.pi > 0.5){
            db.query(`UPDATE "${doc1._key}" WITH {  origin1: 1} in otl_parasites_nodes_nowein`);
            db.query(`UPDATE "${doc2._key}" WITH {  origin2: 1,
                                                    originID: ${counter}} in otl_parasites_nodes_nowein`);
            counter++;}
        if (doc1.pi > 0.5 && doc2.pi < 0.5){
            
            db.query(`UPDATE "${doc1._key}" WITH {  loss1: 1} in otl_parasites_nodes_nowein`);
            db.query(`UPDATE "${doc2._key}" WITH {  loss2: 1,
                                                    lossID: ${counter2}} in otl_parasites_nodes_nowein`);
            counter2++;}
        }
